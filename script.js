...
fetch("livres.json")
  ...
  .then((data) => {
    ...
    livres.forEach((livre) => {
      //Ma div qui contient mes livres
      const livresDiv = document.createElement("div");
      livresDiv.className = "livre";
      livresDiv.innerHTML = `<h3>${livre.titre}</h3>
        <p><strong>Auteur :</strong> ${livre.auteur}</p>
        <p><strong>Année :</strong> ${livre.annee}</p>
        <p><strong>Genre :</strong> ${livre.genre}</p>`;
      catalogue.appendChild(livresDiv);
    });
  });

searchInput.addEventListener("input" ,searchLivres)
function searchLivres (){
  const filter = searchInput.value.toLowerCase();
  const livres = document.querySelectorAll(".livre");

  livres.forEach(livre => {
    const text = livre.textContent.toLowerCase();
    livre.style.display = text.includes(filter) ? "block" : "none"
  });
}